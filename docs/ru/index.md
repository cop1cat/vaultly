...
- Типы соответствуют ожиданиям: `cfg.db_password` — обычный `str`,
  `cfg.max_conns` — обычный `int`.
- Драйверы БД, HTTP-клиенты и Redis работают без обёрток.
- Секреты не утекают в вывод: `repr`, `str`, `model_dump` и JSON-вывод
  отдают `"***"` вместо значения.
- Клонирование запрещено: `pickle`, `copy.copy`, `copy.deepcopy` и
  `model_copy` отказываются работать с моделью, в которой лежит
...
1.0. Публичный API стабилен; ломающие изменения будут идти через смену
мажорной версии по semver. Часть внутренних деталей (в первую очередь
сигнатура `Backend.get`) — кандидаты на изменение в следующем мажоре.
См. [changelog](https://github.com/cop1cat/vaultly/blob/main/CHANGELOG.md).
...
- Запускаете в прод? → [Модель безопасности](guides/security-model.md) и
